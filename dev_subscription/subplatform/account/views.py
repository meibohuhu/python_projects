...
# Create your views here. to connect HTML with backend APIs
...
def register(request):
    ...
    if request.method == "POST":    ## expect POST request, if we're getting a Post request from our HTML page, continue the process
        ...
        if form.is_valid():
            ...
            message = render_to_string('account/email-verification.html', {     ## which is email template sent to user
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None and user.is_writer == True:
                ...
                return redirect('writer-dashboard')      ## in the writer
            if user is not None and user.is_writer == False:
                login(request, user)
                # return HttpResponse('Welcome client!')
                return redirect('client-dashboard')    ## in the client
