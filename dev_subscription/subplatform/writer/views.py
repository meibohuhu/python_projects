...
def create_article(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            article.user = request.user     ### set up user from request.user
            article.save()
            return redirect('my-articles')

    context = {'CreateArticleForm': form}
    return render(request, 'writer/create-article.html', context)   ## what you fill in will be shown in HTML
...
def update_article(request, pk):
    ...
    try:
        article = Article.objects.get(id=pk, user=request.user)    ## get articles for particular user
    except:
        return redirect('my-articles')

    form = ArticleForm(instance=article)    ## retrive that object then getting into HTML(prefilled), if not put instance here, the HTML will be  blank
    if request.method == 'POST':
...
def delete_account(request):
    ...
    if request.method == 'POST':
        ...
        deleteUser.delete()
        return redirect('my-login')       ## redirect to url in account/views we have path('my-login', views.my_login, name="my-login"),
